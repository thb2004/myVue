			...
			<div class="main-middle">
				<el-tabs v-model="activeName">
					<el-tab-pane label="批量增加IP" name="one">
						<el-form :model='oneForm' ref='oneForm' :label-width='$store.state.labelWidth' :rules='rules' :style='{"padding-right": $store.state.labelWidth}'>
							<el-row>
								<el-col :md='12'>
					...
					<el-tab-pane label="批量增加手工IP" name="two">
						<el-form :model='twoForm' :label-width='$store.state.labelWidth' ref='twoForm' :rules='rules' :style='{"padding-right": $store.state.labelWidth}'>
							<el-row>
								<el-col>
					...
					<el-tab-pane label="批量增加主机名" name="three">
						<el-form :model='threeForm' ref="threeForm" :label-width='$store.state.labelWidth' :rules='rules' :style='{"padding-right": $store.state.labelWidth}'>
							<el-row>
								<el-col :md='12'>
									<el-form-item label="所在区域" prop='region'>
	...
	export default {
		...
		methods: {
			...
			tdClick ({row, index, column, tableIndex}) {
				...
				switch (column.label) {
					case '网段':
						app.go({path: 'IPList', query: {ipaddress: row.op_name}})
						break;
					case 'VLAN':
						app.go({path: 'netVlanList', query: {ipaddress: row.op_name}})
						break;
					case '使用IP数':
						app.go({path: 'netVlanList', query: {ipaddress: row.op_name}})
						break;
