...
<style lang='scss'>
	...
	.my-matters {
		...
		.el-tabs__header, .compenent-form, .title-wraper, .el-tab-pane {
	...
	export default {
		data () {
			return {
				...
				stepList: [],
				myMattersTotal: 0,
				myMattersPageSize: 10,
				myMattersCurrentPage: 1,

				myApprovalPageSize: 10,
				myApprovalTotal: 10,
		...
		methods: {
			...
			goProcessPage ({row, index, column}) {
				if (column.label === '操作') {
					app.go({path: 'processingList', query: {ticketNumber: row.ticketNumber, taskId: row.taskId}})
				}
			},
			viewDetail ({row, type}) {
